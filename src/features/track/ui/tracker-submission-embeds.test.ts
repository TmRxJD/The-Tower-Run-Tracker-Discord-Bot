...
describe('tracker submission embeds', () => {
  ...
  it('does not treat destroyed-by-orbs as enemies-hit-by-orbs coverage', () => {
    const embed = buildSubmissionResultEmbed({
      data: {
        tierDisplay: '12+',
        wave: 3456,
  ...
  it('shows coverage for canonical orb-hit and summoned-enemy fields', () => {
    const embed = buildSubmissionResultEmbed({
      data: {
        ...
        type: 'Farming',
        totalEnemies: '5000',
        enemiesHitByOrbs: '1200',
        guardianSummonedEnemies: '300',
      },
      isUpdate: false,
      runTypeCounts: { Farming: 1 },
      hasScreenshot: false,
    ...
    expect(embed.data.fields?.some(field => String(field.value).includes('Orbs: 24%'))).toBe(true)
