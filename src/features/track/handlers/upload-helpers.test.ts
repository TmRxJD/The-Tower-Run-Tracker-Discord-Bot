...
import { findPotentialDuplicateRun, generateCoverageDescription, parseRunDataFromText } from './upload-helpers';

describe('upload-helpers single-line paste parsing', () => {
  it('uses shared duration-aware duplicate identity when matching existing runs', () => {
    expect(findPotentialDuplicateRun(
      {
        tier: '11',
        wave: '7676',
        duration: '9h 54m 5s',
    ...
    expect(findPotentialDuplicateRun(
      ...
      isDuplicate: true,
      duplicateRunId: 'run-fallback',
      duplicateLocalId: 'local-fallback',
    });
